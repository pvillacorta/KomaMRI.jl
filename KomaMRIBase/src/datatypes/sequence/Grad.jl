...
mutable struct Grad
    ...
    first
    last
    function Grad(A, T, rise, fall, delay, first, last)
        all(T .< 0) || rise < 0 || fall < 0 || delay < 0 || first < 0 || last < 0 ? error("Gradient timings must be positive.") : new(A, T, rise, fall, delay, first, last)
    end
